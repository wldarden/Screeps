...
module.exports.findSpacesAround = function(pos) {
    ...
    if (pos.y !== 0) {
        if (pos.x !== 0 && terrain.get(pos.x - 1,pos.y - 1) !== 1) {
            result.push([pos.x - 1, pos.y - 1])
        }
        if (terrain.get(pos.x,pos.y - 1) !== 1) {
            result.push([pos.x, pos.y - 1])
        }
        if (pos.x !== 49 && terrain.get([pos.x + 1, pos.y - 1]) !== 1) {
            result.push( [pos.x + 1, pos.y - 1])
        }
    }
    if (pos.x !== 0 && terrain.get(pos.x - 1, pos.y) !== 1) {
        result.push( [pos.x - 1, pos.y])
    }
    if (pos.x !== 49 && terrain.get(pos.x + 1, pos.y) !== 1) {
        result.push( [pos.x + 1, pos.y])
    }
    if (pos.y !== 49) {
        if (pos.x !== 0 && terrain.get(pos.x - 1, pos.y + 1) !== 1) {
...
module.exports.findDropoff = function(from, base, amount) {
    var structure = findClosestStructureByRange(from, base.dropoffs, (x) => {
        if (x.store) {
            console.log('Checking x.store: ', JSON.stringify(x.store))
            let sum = 0;
            Object.keys(x.store).forEach(key => {
                sum += x.store[key];
            });
            return sum
        } else {
...
module.exports.findStorage = function(from, base, amount, allowSpawn) {
    ...
    let structure = findClosestStructureByRange(from, base.pickups, (x) => {
        if (x.store)
    ...
    if (allowSpawn) {
        let structure = findClosestStructureByRange(from, base.corePickups, (x) => {
            if (x.store)
                return x.store.energy >= amount;
            else
                return x.energy >= amount;
...
module.exports.serializePos = function (pos) {
...
module.exports.isReserved = function(room) {
    ...
    if (!controller)
        ...
        if (!reservation || reservation.ticksToEnd > 500) {
            return true
        } else {
            return false
        }
