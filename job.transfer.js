...
module.exports.run = function (creep) {
  try {
    ...
    let target = Game.getObjectById(creep.memory.dest)
    if (!target) {
      return
    }
    ...
    switch (actionRes) {
      ...
      case ERR_FULL:
        if (!creep.memory.wait) {
          let newDest = getStepEntityId(getCreepStep(creep, job)) // is another target open?
          if (newDest) {
      ...
      case OK:
        ...
        break
      default:
        console.log('Error: ', creep.name, 'Transfer Action Response not handled: ', actionRes, TRANSFER_ERRORS[actionRes], creep.memory.dest, creep.memory.wait, creep.memory.jobId, JSON.stringify(target))
        break
    }
